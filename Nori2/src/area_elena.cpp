...
public:
	...
	virtual Color3f eval(const EmitterQueryRecord & lRec) const {
		...
		if (lRec.n.dot(-lRec.wi) >= 0.0f) return {0, 0, 0};
	...
	virtual Color3f sample(EmitterQueryRecord & lRec, const Point2f & sample, float optional_u) const {
		if (!m_mesh)
			throw NoriException("There is no shape attached to this Area light!");
		//Step 1: Sample a point in the area emitter using Mesh::samplePosition
    	m_mesh->samplePosition(sample, lRec.p, lRec.n, lRec.uv);	

		// Step 2: Calculate the distance between x and x_l and the cosine of the angle between n_l and wi
	...
	virtual float pdf(const EmitterQueryRecord &lRec) const {
		...
		if (lRec.n.dot(-lRec.wi) > 0.0f){
			...
			float dist = static_cast<float>(pow(lRec.dist, 2));
