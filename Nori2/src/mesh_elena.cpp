...
//to calulate the are of a triangule dependeing on the index
...
//to see if a ray(ray3f) intersects a point in a triangule(index), if the intersect it calculates
//the point of the intersection, u, v, t
...
void Mesh::samplePosition(const Point2f &sample, Point3f &p, Normal3f &n, Point2f &uv) const
{
    ...
    // We interpolate the coordinates of the point based on the barycentric coordinates
    p = barycentric[0] * p0 + barycentric[1] * p1 + (1.0f - barycentric[0] - barycentric[1]) * p2;

    uv = barycentric[0]*uv0 + barycentric[1]*uv1 + (1.0f-barycentric[0]-barycentric[1])*uv2;
    // We calculate the surface normal by interpolating the normals of the vertices
    if(m_N.size()>0){
    Normal3f n0 = m_N.col(i0), n1 = m_N.col(i1), n2 = m_N.col(i2);
    n = (barycentric[0] * n0 + barycentric[1] * n1 + (1.0f - barycentric[0] - barycentric[1]) * n2).normalized();
    }
    else n = (p1-p0).cross(p2-p0).normalized();
